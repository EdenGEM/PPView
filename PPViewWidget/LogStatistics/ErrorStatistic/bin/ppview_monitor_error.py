...
#	sqlstr = 'replace into ppview.ppview_chat_test_summary (type, date , error_id, error_num) select type, date, error_id, count(*) from ppview.ppview_chat_test_log group by error_id,type'
	sqlstr = "replace into ppview.ppview_chat_%s_summary (type, date , error_id, error_num) select type, date, error_id, count(*) from ppview.ppview_chat_%s_log where date = '%s' group by error_id,type;" % (kEnv,kEnv,kDate)
#	return 
	try:
		cur.execute(sqlstr)
		conn.commit()
