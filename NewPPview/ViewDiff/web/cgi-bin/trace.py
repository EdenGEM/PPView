...
if load=="resp":
    sqlstr="select resp from trace_qid where env='%s' and uid='%s' and req_type='%s' and qid=%s;"%(env,uid,Type,qid)
    resp=conn.do(sqlstr)
    result=resp[0]["resp"]
else:
    sqlstr="select req from trace_qid where env='%s' and uid='%s' and req_type='%s' and qid=%s;"%(env,uid,Type,qid)
    req=conn.do(sqlstr)
    result=req[0]["req"]
    if load=="query":
