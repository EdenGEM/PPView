...
k_recv_log_pat = re.compile(r'^\[LOG\]\[([0-9:_]+)\]\[([0-9a-zA-Z_]+)--[0-9a-zA-Z]{0,}\]MQQueryProcessor::ParseWorker')
...
def GetInfo():
	...
	for line in sys.stdin:
		...
		if cost_log_match:
			...
			log_dict["cost"] = cost
			time_log = time.mktime(time.strptime("2018" + time_str,'%Y%m%d_%H:%M:%S'))
...
#		WriteTable([u'类型',u'错误id',u'数量'], rows);
		#平均值
		sql = "select type,error_id, round(avg(tot) / 1000000, 2) from %s where date = '%s' and error_id = '0' group by type;" % (Conf.k_dic_config[k_table], k_date)
		cur.execute(sql)
		conn.commit()
		rows = cur.fetchall()
		for row in rows:
...
def main():
	...
	if len(sys.argv) < 3:
		...
		exit(1)
	k_date = sys.argv[1]
	k_table =sys.argv[2]+"_cost_stats"
