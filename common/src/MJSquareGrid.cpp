...
bool MJSquareGrid::getRangePlace(std::string mapInfo,int Dist,std::tr1::unordered_set<std::string> &idList){
    ...
    pos = key.find("_");
    if(pos == std::string::npos) {
        std::cerr<<"MJSquareGrid::getRangePlace key error"<<std::endl;
        return false;
    }
	...
	//std::cerr<<"getRangePlace: before filter idList size: "<<idList.size()<<std::endl;
