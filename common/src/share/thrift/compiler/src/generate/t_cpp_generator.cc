...
void t_cpp_generator::generate_service(t_service* tservice) {
  ...
  f_service_ << "#include <sys/time.h>" << endl;
  if (gen_cob_style_) {
...
void t_cpp_generator::generate_service_interface(t_service* tservice, string style) {
  ...
  //Mioji add begin -- by shichenxi
  f_header_ << indent() << "virtual void doSomethingBeforeProcess(const void* req, const long& ts, const uint32_t& reqBytes) = 0;" << endl;
  f_header_ << indent() << "virtual void doSomethingAfterProcess(const void* req, const void* resp, const long& ts, const long& cost, const uint32_t& respBytes) = 0;" << endl;
  //Mioji add end
...
void t_cpp_generator::generate_service_client(t_service* tservice, string style) {
  ...
  //Mioji add start -- by shichenxi
  indent(f_header_) << "/*以下几行是Mioji自定义代码*/" << endl;
  indent(f_header_) << "void doSomethingBeforeProcess(const void* req, const long& ts, const uint32_t& reqBytes){}" << endl;
  indent(f_header_) << "void doSomethingAfterProcess(const void* req, const void* resp, const long& ts, const long& cost, const uint32_t& respBytes){}" << endl << endl;
  //Mioji add end
...
void t_cpp_generator::generate_process_function(t_service* tservice,
  ...
  if (style != "Cob") {
    ...
    //Mioji add start -- by shichenxi
    out 
      << indent() << "timeval t1;" << endl
      << indent() << "gettimeofday(&t1,NULL);" << endl
      << indent() << "long tsStart = t1.tv_sec*1000000 + t1.tv_usec;" << endl;
    if (fields.size() != 0)
      out << indent() << "iface_->doSomethingBeforeProcess(&args." << (*fields.begin())->get_name() << ",tsStart,bytes);" << endl;
    else
      out << indent() << "iface_->doSomethingBeforeProcess(NULL,tsStart,bytes);" << endl;
    ...
    out << indent() << "if (this->eventHandler_.get() != NULL) {" << endl << indent()
        ...
        //Mioji add start -- by shichenxi
        << indent() << "timeval t2;" << endl
        << indent() << "gettimeofday(&t2,NULL);" << endl
        << indent() << "long tsEnd = t2.tv_sec*1000*1000 + t2.tv_usec;" << endl;
    if (fields.size() != 0)
      out << indent() << "iface_->doSomethingAfterProcess(&args." << (*fields.begin())->get_name() << ", &result.success,tsEnd,tsEnd-tsStart,bytes);" << endl;
    else
      out << indent() << "iface_->doSomethingAfterProcess(NULL, NULL, tsEnd,tsEnd-tsStart,bytes);" << endl;
        //Mioji add end
    out << indent() << "oprot->getTransport()->flush();" << endl << endl << indent()
        << "if (this->eventHandler_.get() != NULL) {" << endl << indent()
...
void t_cpp_generator::generate_service_skeleton(t_service* tservice) {
  ...
  f_skeleton << "// This autogenerated skeleton file illustrates how to build a server." << endl
             ...
             << "#include <thrift/server/TNonblockingServer.h>" << endl
             << "#include <thrift/concurrency/PosixThreadFactory.h>" << endl
             << "#include <thrift/transport/TServerSocket.h>" << endl
             << "#include <thrift/transport/TBufferTransports.h>" << endl << endl
             //Mioji add start by shichenxi
             << "#include \"MJCommon.h\"" << endl
             << "#include \"malloc.h\"" << endl
             << "#include \"signal.h\"" << endl << endl 
             //Mioji add end
             << "using namespace ::apache::thrift;" << endl
             << "using namespace ::apache::thrift::protocol;" << endl
             << "using namespace ::apache::thrift::transport;" << endl
             << "using namespace ::apache::thrift::concurrency;" << endl
             << "using namespace ::apache::thrift::server;" << endl << endl
             << "using namespace boost;" << endl << endl;
  ...
  //Mioji add begin -- by shichenxi
  f_skeleton << indent() << "/*收到请求时记录Mioji日志用，除非必要，否则不要修改。如果编译不过去，请检查是否使用了globals.thrift*/" << endl;
  f_skeleton << indent() << "void doSomethingBeforeProcess(const void* req, const long& ts, const uint32_t& reqBytes){" << endl;
  indent_up();
  f_skeleton << indent() << "if (req == NULL) return;" << endl;
  f_skeleton << indent() << "const MJRequest* mjReq = static_cast<const MJRequest*>(req);" << endl;
  f_skeleton << indent() << "MJ_LOG_REQUEST(\"type=%s,uid=%s,csuid=%s,qid=%s,ts=%ld,querystring=%s,ret=%d\",mjReq->api.type.c_str(),mjReq->api.uid.c_str(),mjReq->api.csuid.c_str(),mjReq->api.qid.c_str(),ts,mjReq->query.c_str(),reqBytes);" << endl;
  indent_down();
  f_skeleton << indent() << "}" << endl;
  f_skeleton << indent() << "/*请求返回时记录Mioji日志用，除非必要，否则不要修改。如果编译不过去，请检查是否使用了globals.thrift*/" << endl;
  f_skeleton << indent() << "void doSomethingAfterProcess(const void* req, const void* resp, const long& ts, const long& cost, const uint32_t& respBytes){" << endl;
  indent_up();
  f_skeleton << indent() << "if (req == NULL) return;" << endl;
  f_skeleton << indent() << "const MJRequest* mjReq = static_cast<const MJRequest*>(req);" << endl;
  f_skeleton << indent() << "MJ_LOG_RESPONSE(\"type=%s,uid=%s,csuid=%s,qid=%s,ts=%ld,cost=%ld,ret=%d\",mjReq->api.type.c_str(), mjReq->api.uid.c_str(), mjReq->api.csuid.c_str(), mjReq->api.qid.c_str(), ts, cost, respBytes);" << endl;
  indent_down();
  f_skeleton << indent() << "}" << endl;
  //Mioji add end
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    indent_up();
    f_skeleton     
        << indent() << "//业务代码，请在此处实现自己的业务逻辑" << endl;
    indent_down();
    //Mioji add end
    f_skeleton 
        << indent() << "}" << endl << endl;
  ...
  f_skeleton 
      << indent() << "void sigterm_handler(int signo) {}" << endl 
      << indent() << "void sigint_handler(int signo) {}" << endl
      << indent() <<"int main(int argc, char **argv) {" << endl;
  indent_up();
  f_skeleton
      << indent() << "/*模板代码-无需修改*/" << endl
      << indent() << "mallopt(M_MMAP_THRESHOLD, 64*1024);" << endl
      << indent() << "close(STDIN_FILENO);" << endl
      << indent() << "signal(SIGPIPE, SIG_IGN);" << endl
      << indent() << "signal(SIGTERM, &sigterm_handler);" << endl
      << indent() << "signal(SIGINT, &sigint_handler);" << endl
      << indent() << "mallopt(M_MMAP_THRESHOLD, 64*1024);" << endl << endl <<endl
      << indent() << "/*业务代码--仅在此处替换自己的业务逻辑*/" << endl << endl <<endl
      << indent() << "/*模板代码-请指定端口号和业务线程数*/" << endl
      << indent() << "int port = 9090;" << endl
      << indent() << "int thread_num = 10;" << endl << endl
      << indent() << "/*模板代码-无需修改（除非对服务端有特殊需求）*/" << endl
      << indent() << "////TSimpleServer模式用/////" << endl
      ...
      << indent() << "////TNonblocking && THsHaServer////" << endl
      << indent() << "shared_ptr<" << svcname << "Handler> handler(new " << svcname << "Handler());" << endl
      << indent() << "shared_ptr<TProcessor> processor(new " << svcname << "Processor(handler));" << endl
      << indent() << "shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());" << endl
      << indent() << "shared_ptr<ThreadManager> threadManager = ThreadManager::newSimpleThreadManager(thread_num);" << endl
      << indent() << "shared_ptr<PosixThreadFactory> threadFactory = shared_ptr<PosixThreadFactory>(new PosixThreadFactory());" << endl
      << indent() << "threadManager->threadFactory(threadFactory);" << endl
      << indent() << "threadManager->start();" << endl
      << indent() << "TNonblockingServer server(processor,protocolFactory,port,threadManager);" << endl
