...
std::bitset<32> PathGenerator::countOpenClose(Json::Value routeJ){
//    int count=0;
	std::bitset<32> b;
        ...
        if (NodeJ::isFixed(routeJ[i])) {
            if(i-fixId>1){
                ...
                for(int j=fixId;j<=i;j++){
                    segmentRouteJ.append(routeJ[j]); //按锁定点将路线分段
                }
                ...
                for(int j=1; j<segmentRouteJ.size(); j++){
                    for(int k=0;k<routeJList.size();k++){
                        ...
                        ResultRouteJList.push_back(tempRouteJ);//放j点默认不关心开关门路线
                        ...
                        for(int z=0;z<OpenClose.size();z++){
                            ...
                            if(isOpen(tempRouteJ[j],z)) {
                                ResultRouteJList.push_back(tempRouteJ);//放j点选z开门区间路线
                                break;
							}
							if (openCloseT.cost()/1000 > 3000) {
								TimeOut=true;
							}
							if(TimeOut) break;
                        }
                    ...
                    routeJList.clear();
                    routeJList=ResultRouteJList;//处理完j点后可能的所有路线
                ...
                for(int v=len-1;v>=0;v--){
					std::bitset<32> b=countOpenClose(routeJList[v]);
					int bNum=b.count();
					if(bNum>maxCount) {
						maxCount=bNum;
						select=v;
				...
				segmentRouteJ=routeJList[select];//选择开门数尽量多，且开门点尽量靠前点路线（有多条开门的数量相同的路线时）
                if(debugInfo)
